...
def main():
    ...
    server.add_api_route("/answers/", numenex_module.get_answers, methods=["GET"])

    @server.patch("/answers/")
    async def validate_and_update_answers(answers: ty.List[ty.Dict[str, ty.Any]]):
        score_dict = {}
        ...
        return numenex_module.answer_questions(
            data=answers, method="patch", path="answers"
        )
